...
This is Part 12 of a 15-part [Apache Iceberg Masterclass](../README.md). [Part 11](/blog/2026-04-29-iceberg-masterclass-11-metadata-tables.html) covered metadata tables. This article covers the two main ways to access Iceberg data: directly from Python libraries and through MPP (massively parallel processing) query engines.
...
![How Python libraries and MPP engines connect to Iceberg tables](/assets/blog/iceberg-masterclass/12-python-and-mpp-python-iceberg-stack.png)
...
![The five-step PyIceberg workflow from catalog connection to analysis](/assets/blog/iceberg-masterclass/12-python-and-mpp-pyiceberg-workflow.png)

PyIceberg leverages Iceberg's [metadata-driven pruning](/blog/2026-04-29-iceberg-masterclass-03-iceberg-metadata-performance.html): the `row_filter` is pushed down to manifest evaluation, so only relevant data files are read. For reading subsets of large tables into Python for analysis or ML training, this is remarkably efficient.
...
DuckDB also supports reading Iceberg metadata tables, which means you can use it for [table health diagnostics](/blog/2026-04-29-iceberg-masterclass-09-storage-degradation.html) without standing up a full query engine.
...
This creates a new Iceberg [commit](/blog/2026-04-29-iceberg-masterclass-06-writing-to-iceberg.html) with the data files, manifests, and metadata. PyIceberg handles the entire write lifecycle, including partition assignment based on the table's [partition spec](/blog/2026-04-29-iceberg-masterclass-05-hidden-partitioning.html).
...
![Comparison of MPP engines for Iceberg workloads showing read, write, and maintenance capabilities](/assets/blog/iceberg-masterclass/12-python-and-mpp-mpp-engine-comparison.png)
...
Apache Spark is the most mature Iceberg engine for both reads and writes. It handles batch ETL, streaming ingestion ([Part 13](/blog/2026-04-29-iceberg-masterclass-13-streaming-to-iceberg.html)), and all [maintenance operations](/blog/2026-04-29-iceberg-masterclass-10-maintaining-iceberg.html). Most Iceberg production pipelines use Spark for data ingestion because of its extensive connector ecosystem (Kafka, JDBC, file formats) and its ability to process large volumes across a distributed cluster.
...
[Part 13](/blog/2026-04-29-iceberg-masterclass-13-streaming-to-iceberg.html) covers how to stream data into Iceberg tables.
